...
import ContextMenuProvider from './menus/context-menu-provider'
...
export function activate(context: ExtensionContext) {
  ...
  const contextMenuProvider = new ContextMenuProvider(extension)
  ...
  workspace.onDidChangeConfiguration(event => {
    if (event.affectsConfiguration('yajer.jestConfigPatterns')) {
      Jest.getConfigs(true)
    }
  })
}
